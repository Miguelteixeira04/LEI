globals [
  ...
  cleaner-energy-blue   ; energia do Cleaner
  max-cleaner-debris    ; capacidade máxima de detritos que o Cleaner pode carregar
  current-debris-count-blue  ; contagem de detritos que o Cleaner está transportando
...
to setup
  ...
  ; Depois cria o Cleaner para garantir que se sobreponha ao Charger
  create-turtles 1 [
  ...
  ; Criando 3 Polluters, cada um com um tipo de detrito exclusivo
  create-turtles 1 [
...
to go
  while [cleaner-energy-blue > 0] [
    go_once
    ; Atualiza a energia e pode incluir outros comportamentos adicionais
  ]
  stop  ; Para o modelo quando a energia do Cleaner atinge 0
end
...
to move
  let trash one-of turtles with [role = "Trash"]  ; Identifica a lixeira (Trash)
  let charger one-of turtles with [role = "Charger"] ; Identifica o Charger

  ; Reduz a energia a cada movimento, se for o Cleaner
  if role = "Cleaner" [
...
to recharge
  ; O Cleaner vai carregar se a energia estiver abaixo de 30
...
to safe-spawn
  ...
  while [any? other turtles in-radius 2 or (charger != nobody and distance charger < 2)] [
    limit
    setxy random-xcor random-ycor
    limit
  ]
