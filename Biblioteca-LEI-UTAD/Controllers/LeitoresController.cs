...
namespace Trabalho.Controllers
{
    ...
    public class LeitoresController : Controller
    {
        ...
        public async Task<IActionResult> Requisitar([FromBody] Requisicao model)
        {
            ...
            var maxIdRequisicao = await _context.Requisicaos.MaxAsync(r => (int?)r.IdRequisicao) ?? 0;
            var nextIdRequisicao = maxIdRequisicao + 1;

            if (nextIdRequisicao > 99999)
            {
                return Json(new { sucesso = false, mensagem = "O limite de requisições foi atingido." });
            }
