...
class Wavy
{
    ...
    static void Main()
    {
        string wavyID = "WavyA1";
        string agregadorID = "Agregador 123";
        cliente = new TcpClient("127.0.0.1", 4000);
        stream = cliente.GetStream();

        // Caminho para a pasta "Data source" dentro do repositório
        string basePath = "C:/Users/mikeb/source/repos/SD_PL3_78321_78282_78806/Wavy/Data";
    ...
    static void EnviarDados()
    {
        ...
        int slotInCycle = 0; // 0 = first position
        ...
        {
            // Se recebemos uma atualização de status, precisamos esperar que o agregador 
            // termine de enviar todas as atualizações de status
            if (statusRecemAtualizado)
            {
                Console.WriteLine("Aguardando sincronização com outras Wavys...");
                Thread.Sleep(5000); // Tempo suficiente para que o agregador envie status para todas as Wavys
                statusRecemAtualizado = false;
                lastSendTime = DateTime.Now; // Reset timing
            }

            // Check if we're paused or not operational
            if (aguardandoRetomada || estadoAtual != "Operacional")
            {
                // If we're paused or in maintenance or any other non-operational state, just wait and check again
                Thread.Sleep(1000);

                // When we detect that we're no longer paused and operational, set the resuming flag
                if (!aguardandoRetomada && estadoAtual == "Operacional")
                {
            ...
            // If we get here, we're operational and not paused
            // Calculate time since last send to maintain cycle timing
            TimeSpan timeSinceLastSend = DateTime.Now - lastSendTime;

            // If resuming from pause, reset timing
            if (resumingFromPause)
            {
                Console.WriteLine("Aguardando confirmação de que todas as Wavys receberam as suas atualizações de estado...");
                Console.WriteLine("-------------------------------------------------\n");

                Thread.Sleep(2000); // Give more time for all Wavys to process their status updates

                resumingFromPause = false;
                // WavyA1 goes first after pause
                Thread.Sleep(1000); // Small delay to ensure all Wavys have processed the resume
            }
            // Otherwise, wait until our slot in the cycle
            else if (timeSinceLastSend.TotalMilliseconds < cycleTime)
            {
                // Wait until the next complete cycle
                int waitTime = (int)(cycleTime - timeSinceLastSend.TotalMilliseconds);

                // Break up the wait into smaller chunks so we can check for pause
                ...
                while (remainingWait > 0 && !aguardandoRetomada)
                {
                    ...
                    // If we got paused during the wait, break out
                ...
                // If we got paused during the wait, skip to next iteration
                if (aguardandoRetomada)
                {
                    continue;
                }
            ...
            // Check again right before sending data
            if (aguardandoRetomada)
            {
                continue;
            }

            // Set the flag that we're sending data
            sendingData = true;
            ...
            // Check one more time before actually sending
            if (!aguardandoRetomada)
            {
